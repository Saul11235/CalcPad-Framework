...
#   #def SVG_poly2$(draw_X1$; draw_Y1$; draw_X2$; draw_Y2$)
#   #val
#   #show
#   '<polygon points="'SVG_pointerX','SVG_pointerY' 'SVG_pointerX + (draw_X1$)*SVG_scaleFactor','SVG_pointerY + (draw_Y1$)*SVG_scaleFactor'  'SVG_pointerX + (draw_X1$ + draw_X2$)*SVG_scaleFactor','SVG_pointerY + (draw_Y1$ + draw_Y2$)*SVG_scaleFactor'" style="stroke:'SVG_getcolorLine$'; stroke-width:'SVG_LineWidth'; fill:'SVG_getcolorBackground$'; fill-opacity:'SVG_BackgroundOpacity'; stroke-opacity:'SVG_LineOpacity'" />
#   #val
#   #hide
...
def get_code(order):
    ...
    code="#def SVG_poly"+strorder+"$("
    l=[]
    for x in range(order):
        l.append("draw_X"+str(int(x+1))+"$")
        l.append("draw_Y"+str(int(x+1))+"$")
    code+="; ".join(l)+")\n  #val\n  #show\n  "
    code+="""'<polygon points="'SVG_pointerX','SVG_pointerY' """
    puntos=[]
    for x in range(order): puntos.append(get_punto(x))
    code+=" ".join(puntos)
    code+=""" " style="stroke:'SVG_getcolorLine$'; stroke-width:'SVG_LineWidth'; fill:'SVG_getcolorBackground$'; fill-opacity:'SVG_BackgroundOpacity'; stroke-opacity:'SVG_LineOpacity'" />"""
